...
| `tsconfig.json` | TypeScript config (extends `src/valdi_modules/src/valdi/_configs/base.tsconfig.json`) |
...
```bash
cd client/src/open_source/.claude/skills/valdi-migrate/tests
...
bzl build //.claude/skills/valdi-migrate/tests:tests
```
